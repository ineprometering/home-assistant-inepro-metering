...
async def test_gateway_network_scan_allows_explicit_target_retry(
    ...
    with patch(
        "custom_components.inepro_metering.config_flow.async_discover_tcp_gateways",
...
async def test_gateway_can_be_added_when_no_downstream_meters_are_found(
    ...
    with patch(
        "custom_components.inepro_metering.config_flow.async_discover_grow_tcp_gateway",
        new=AsyncMock(return_value=()),
    ):
        result = await hass.config_entries.flow.async_configure(
            result["flow_id"],
...
async def test_grow_serial_scan_discovers_and_creates_entry(
    ...
    with patch(
        "custom_components.inepro_metering.config_flow.async_discover_grow_serial_bus",
...
async def test_validate_entry_identity_uses_stored_serial_number(
    ...
    with patch(
        "custom_components.inepro_metering.config_flow.IneproModbusClient",
        return_value=client,
    ):
...
async def test_options_submit_preserves_active_route_and_deduplicates_routes(
    ...
    with (
        patch(
            "custom_components.inepro_metering.build_runtime_coordinator",
