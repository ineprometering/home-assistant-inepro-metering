...
def test_get_active_route_for_meter_falls_back_to_first_route_when_key_is_stale() -> None:
...
def test_ensure_bus_meter_routes_adds_missing_bus_route_without_losing_active_route() -> None:
...
def test_with_routes_applied_mirrors_selected_active_route_to_top_level_fields() -> None:
...
def test_normalize_entry_route_data_collapses_duplicate_route_keys() -> None:
    ...
    deduped_route = next(
        route
        for route in updated[CONF_ROUTES]
        if route[CONF_HOST] == "192.0.2.10"
    )
...
def test_normalize_entry_route_data_keeps_active_alternate_direct_tcp_route() -> None:
    ...
    assert updated[CONF_HOST] == "192.0.2.14"
    assert updated[CONF_PORT] == 502
